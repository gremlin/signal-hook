...
//! are the minor price around signals anyway. But the code states which orderings should be enough
//! in practice in case someone wants to get inspired (but do make your own check through them
//! anyway).
...
impl<T> HalfLock<T> {
    ...
    pub(crate) fn replace<F, E>(&self, update: F) -> Result<(), E>
    where
        F: FnOnce(&T) -> Result<T, E>,
    {
        // While it's possible the user callback panics, our code doesn't and the data gets swapped
        // atomically. So if it panics, nothing gets changed, therefore poisons are of no interest
        // here.
        let _guard = self
            .write_mutex
            .lock()
        ...
        // Get a new instance with updated data.
        let new = update(data)?;
        ...
        let old = self.data.swap(new, Ordering::SeqCst);

        // Now we make sure there's no reader having the old data.
        self.write_barrier();

        // Valid for the same reason as above. And we claimed ownership.
        // Will get dropped at the end of this function. After we make sure it is actually safe to
        // do.
